...
/// @dev Calls notify() on the victim during its transfer(), triggering reentrancy.
...
/// @dev Exposes internal _msgSender / _msgData for EIP-2771 branch coverage.
...
contract BusinessWalletTest is Test {
    ...
    function test_msgSender_nonForwarder_returnsMsgSender() public {
        ...
        bytes memory init = abi.encodeCall(BusinessWallet.initialize, (BID, admin, pool, address(0), address(0)));
    ...
    function test_msgSender_viaForwarder_extractsAppendedSender() public {
        ...
        BWHarness impl2 = new BWHarness();
        UpgradeableBeacon b = new UpgradeableBeacon(address(impl2));
        bytes memory init = abi.encodeCall(BusinessWallet.initialize, (BID, admin, pool, fwd, address(0)));
        BWHarness harness = BWHarness(payable(address(new BeaconProxy(address(b), init))));
